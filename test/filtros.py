...
from PyQt5.QtCore import QTimer, QDate, QRect, pyqtSignal
...
class TicketManagement(QMainWindow):
    ...
    def log_incidence_to_excel(self, block_name, date_str, time_str, incidence_text):
        if self.excel_file and os.path.exists(self.excel_file):
            ...
            # Verificar si la primera fila tiene los encabezados correctos y actualizar si es necesario
                if sheet.max_row == 1:  # Si solo hay una fila, probablemente sea la cabecera
                    headers = [cell.value for cell in sheet[1]]
                    expected_headers = ['Fecha', 'Hora'] + self.blocks
                    if headers != expected_total_headers:
                    # Si no coincide, insertamos una fila nueva con los encabezados correctos
                        sheet.insert_rows(1)
                        for i, header in enumerate(expected_headers):
                            sheet.cell(row=1, column=i+1, value=header)

                # Agregar la nueva fila con la incidencia
                new_row_index = sheet.max_row + 1
                new_row = [date_str, time_str] + ['-' for _ in self.blocks]
                block_index = self.blocks.index(block_name) + 3  # +3 porque los índices comienzan en 1 y ya hay 2 columnas para fecha y hora
                ...
                QMessageBox.critical(self, "Error", f"No se pudo registrar la incidencia en Excel: {e}")
    ...
    def update_graph(self, data=None):
        if data is None:  # Si no se pasan datos, carga todos los datos del archivo Excel
            if self.excel_file and os.path.exists(self.excel_file):
                workbook = load_workbook(self.excel_file)
                sheet = workbook.active
        ...
        blocks = [block.split()[0] for block in self.blocks]  # Acortando nombres de bloques para la visualización
        ax.bar(blocks, [self.incidences_count[block] for block in self.blocks])
    ...
    def confirm_incidence(self, block_name, list_widget):
        ...
        if current_item:
            ...
            self.global_incidence_list.addItem(f"{block_name}: {incidence_text} a las {time_str} del {date_str}")
            self.update_excel_table()
            self.update_graph()  # Actualiza con todos los datos para ver el cambio inmediatamente
        else:
            QMessageBox.warning(self, "Ninguna Incidencia Seleccionada", "Selecciona una incidencia para confirmar.")
    ...
    def apply_filters(self):
        ...
        if self.excel_file and os.path.exists(self.excel_file):
            ...
            self.update_excel_table(filtered_incidences)
            self.update_graph(filtered_incidences)  # Actualiza con datos filtrados
